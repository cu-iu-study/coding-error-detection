...
# Directory containing all Python adapter test resources.
TEST_RESOURCES_DIRECTORY: Path = Path(__file__).parent / "resources" / "javascript"

# Test resource file names.
ASSIGNMENTS_SOURCE_FILE: str = "assignments.js"
FUNCTION_CALLS_SOURCE_FILE: str = "function_calls.js"
MIXED_SOURCE_FILE: str = "mixed.js"
EMPTY_SOURCE_FILE: str = "empty.js"
SYNTAX_ERROR_SOURCE_FILE: str = "syntax_error.js"
...
def test_extract_assignments(parser: Parser, adapter: Adapter) -> None:
    ...
    assert third_assignment.assigned_value.source_text == "token.trim()"
...
def test_extract_function_call_arguments(parser: Parser, adapter: Adapter) -> None:
    ...
    assert function_call.arguments[2].source_text == "token.trim()"
...
def test_source_positions(parser: Parser, adapter: Adapter) -> None:
    ...
    assert first_assignment.assigned_variable.variable_name == "password"
    assert first_assignment.line_number == 1
    assert first_assignment.column_number == 7

    second_assignment = analysis_context.assignments[1]

    assert second_assignment.assigned_variable.variable_name == "token"
    assert second_assignment.line_number == 3
    assert second_assignment.column_number == 7

    third_assignment = analysis_context.assignments[2]

    assert third_assignment.assigned_variable.variable_name == "secret"
    assert third_assignment.line_number == 5
    assert third_assignment.column_number == 7

    eval_call = analysis_context.function_calls[1]

    assert eval_call.function_name == "eval"
    assert eval_call.line_number == 7
    assert eval_call.column_number == 1

    system_call = analysis_context.function_calls[2]

    assert system_call.qualifier == "os"
    assert system_call.function_name == "system"
    assert system_call.line_number == 9
    assert system_call.column_number == 1

    run_call = analysis_context.function_calls[3]

    assert run_call.qualifier is None
    assert run_call.function_name == "run"
    assert run_call.line_number == 11
    assert run_call.column_number == 1

    string_literal = run_call.arguments[0]

    assert isinstance(string_literal, StringLiteral)
    assert string_literal.line_number == 12
    assert string_literal.column_number == 5

    variable_reference = run_call.arguments[1]

    assert isinstance(variable_reference, VariableReference)
    assert variable_reference.line_number == 13
    assert variable_reference.column_number == 5

    unknown_expression = run_call.arguments[2]

    assert isinstance(unknown_expression, UnknownExpression)
    assert unknown_expression.line_number == 14
    assert unknown_expression.column_number == 5
